...
from omd2l.utils.io_utils import download, extract
...
class MTFraEng(DataModule):
    ...
    def _download(self):
        extract(download(
            cfg.DATA_URL + 'fra-eng.zip', self.root,
    ...
    def _build_arrays(self, raw_text, src_vocab=None, tgt_vocab=None):
        ...
        def _build_array(sentences, vocab, is_tgt=False):
            ...
            array = compute.tensor([vocab[s] for s in sentences])
            valid_len = compute.reduce_sum(
                compute.astype(array != vocab['<pad>'], compute.int32), 1)
